...
$tempQuery = "SELECT * from product_category where prodcat_id='6'";
...
if ($cat_stat == 'Active') {
    ...
    $query = "SELECT * from product where prodcat_id='6'";
    $run = mysqli_query($conn, $query);
    while ($row = mysqli_fetch_assoc($run)) {
        $path = "../cspManager/" . $row['image'];
        $output1 .= '<div class="card  text-center  ml-3 my-3  " style="max-height: 260px;min-height:260px; border: none !important; max-width:185px;min-width:185px ;display: flex; justify-content: center; align-items:center;">
...
<a href="singleproductpage.php?view=' . $row['product_id'] . '" style="background-color: #52944D; color: white; width: 100px;" class="btn btn-primary btn-sm">View</a>
    ...
    $output2 = '<h5><i class="fa fa-tshirt mr-2 ml-1"> </i>SHOES & GARMENTS</h5>';
} else {
